...
Suffix arrays (SAs) zijn een geheugenefficiëntere voorstelling van de bladeren van een suffixbomen.
In plaats van een boomstructuur maken ze gebruik van \textbf{een array die de volgnummers van elke suffix in de originele string bevat}.
...
Om een indicatie te hebben van de extra hoeveelheid geheugen die nodig is om gebruik te maken van Enchanced Suffix Arrays kunnen we via de libsais bibliotheek de pLCP en LCP array berekenen.
...
\textbf{Het berekenen van deze extra arrays vraagt dus ongeveer dubbel zoveel geheugen, waardoor we deze optie niet verder uitwerken}.
Bovendien berekenen we deze LCP array om zo het voorberekenen van de geaggregeerde taxon ID voor elke interne top van de overeenkomstige suffixboom te faciliteren.
Om dit effectief mogelijk te maken zouden we dus op basis van de SA en LCP nog een compacte representatie van de boomstructuur moeten vinden
...
Daarna moet voor elk van deze matches het niet-gematchte deel van de peptide nog gecheckt worden.
\\ \\
...
Om alles iets duidelijker te maken, volgt een klein voorbeeld waarin we alle voorkomens van de peptide \texttt{acg} in de tekst \texttt{acacgt\$} zoeken.
De index die gebruikt wordt om te zoeken is een SSA met sparseness factor $k$ = 3 (Figuur~\ref{fig:sparse_sa}).
...
Op het eerste gezicht lijkt het vergroten van deze factor enkel meer iteraties te vragen, maar dit heeft zware gevolgen.
...
Zeker wanneer de zoekstrings zelf al vrij kort zijn, wat in onze toepassing typisch het geval is.
...
\begin{figure}[H]
    \centering
    \subfloat[Zoektijd voor het Swiss-Prot peptidebestand met en zonder \textit{missed cleavages}. De zoekoperaties zijn uitgevoerd op een sparse suffix array gebouwd op basis van de Swiss-Prot eiwitdatabank, met $k = 1, 2, 3, 4, 5$.]{\includegraphics[width=\linewidth]{swissprot_searchtime_sparseness}}\\[4ex] % [4ex] om wat extra vertical spacing in te voegen
...
\begin{figure}[H]
    \centering
    \subfloat[Absolute uitvoeringstijd voor 1--12 threads.]{\includegraphics[width=0.7\linewidth]{threading_default_vs_rayon}}\\[4ex] % [4ex] om wat extra vertical spacing in te voegen
...
Aangezien er met se SA-indexstructuur geen voorberekening gebeurt van LCA's, is het enkel nuttig om de zoektijd inclusief het berekenen van de LCA te bekijken.
...
Zo zijn er slechts $\pm$ 13\thinspace000 tryptische peptiden die meer dan 10\thinspace000 eiwitten matchen.
...
\begin{figure}[H]
    ...
    \caption{Berekenen van de LCA* (inclusief zoeken) voor alle peptiden (a) zonder en (b) met het gebruik van de drempelwaarde (B=10\thinspace000) op het Human-Prot en Swiss-Prot proteïnenbestand.}\label{fig:cutoff_humanprot}
\end{figure}
