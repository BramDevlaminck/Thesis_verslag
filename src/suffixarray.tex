...
We kunnen concluderen dat libsais duidelijk de snelste implementatie is om swissprot te indexeren.
...
Dit is erg belangrijk voor het indexeren van UniprotKB omdat de totale tekst langer is dan de maximale 32 bit integer.
...
Daarom hebben we gekozen om gebruik te maken van de interoperabiliteit die bestaat tussen Rust en C/C++.
...
Bovendien zijn we ook zeker dat als een geheugenfout gebeurt dat dit aan de basis ervan zal liggen.
...
Om de impact op de zoektijd beter te visualiseren toont figuur~\ref{fig:search_sparseness} de zoektijd van het Swiss-Prot zoekbestand met en zonder missed cleavages.
...
Dit omdat er een deel van de peptides korter zijn dan 5 aminozuren lang, deze zouden dus niet meer gezocht kunnen worden.

\begin{figure}[H]
    \center
    \includegraphics[width=\linewidth]{swissprot_searchtime_sparseness}
    \caption{Zoektijd voor het Swiss-Prot zoekbestand met en zonder missed cleavages in een sparse suffix array gebouwd aan de hand van de Swiss-Prot eiwitdatabank.}
    \label{fig:search_sparseness}
\end{figure}

% TODO: grafiek die SSA size toont voor swissprot met verschillende sparseness factors
...
Wanneer we deze proberen zoeken in een SSA een sparseness factor die slechts een beetje kleiner is of gelijk is hieraan zorgt dit er voor dat het grootste deel van de peptide overgeslagen kan worden als prefix.
Wat op zijn beurt impliceert dat de gezochte suffix extreem kort is, en bijgevolg erg veel matches heeft.
Voor elk van deze matches moeten we controleren als de prefix matcht of niet, wat veel tijd kost.
\\ \\
Er zijn dus twee erg belangrijke bevindingen over sparse suffix arrays, en de manier waarop wij ze opbouwen
\begin{enumerate}
    \item Probeer de sparseness factor zo laag mogelijk te houden zodat de zoektijd voor korte peptides beperkt blijft.
...
Op basis hiervan kunnen we daarna vaststellen welke verbeteringen we gemaakt hebben, als deze indexstructuur goed genoeg schaalt om toepasbaar te zijn op de volledige UniProtKB en in welke tijd de peptides gezocht kunnen worden.
...
Dit is nog steeds een grote hoeveelheid, maar dit wordt al praktisch mogelijk op een krachtige server gericht op memory intensive tasks.
...
\subsection{Zoeken}
Aangezien er geen voorberekening gebeurd van LCA's is er bij het meten van de zoekperformantie geen reden om twee gevallen zoals bij suffixbomen te bekijken.
De enige manier van zoeken waarin we geïnteresseerd zijn is het zoeken van alle matchende proteïnes en dan als resultaat de LCA (of in ons geval de LCA*) als output krijgen.

\subsection{Maximaal aantal matches}\label{subsec:maximaal-aantal-matches}


\begin{figure}[H]
    \centering
    \subfloat[Bereken alle LCAs.]{\includegraphics[width=0.7\linewidth]{no_cutoff_humanprot_search}}\\[4ex] % [4ex] om wat extra vertical spacing in te voegen

    \subfloat[Bereken de LCA als er minder dan 10000 matches zijn, geef anders root terug als antwoord.]{\includegraphics[width=0.7\linewidth]{cutoff_humanprot_search}}
    \caption{cut-off en geen cut-off op Human-Prot bij het zoeken naar de LCA voor alle peptides}\label{fig:cutoff_humanprot}
\end{figure}

Zoals in figuur~\ref{fig:cutoff_humanprot} duidelijk te zien valt is er een gigantisch verschil tussen het gebruikmaken van een cut-off of geen cut-off.
Hierbij worden alle peptides die meer dan $x$ matches hebben verondersteld als LCA de root te hebben.
We doen dit omdat blijkt dat peptides met erg veel matches voor een sterke meerderheid van de gevallen een erg algemeen resultaat opleveren\cite{unipept_cutoff}.
Langs de andere kanten is het berekenen van de LCA net erg traag voor deze gevallen.
...
Van de 100000 peptides zijn er 12 peptides die een ander resultaat verkrijgen in de output.
Deze 12 entries zijn echter slechts 2 verschillende peptides (die gewoon meerdere malen voorkomen in het peptidebestand).
...
\\ \\
Indien we het gebruik van de cut-off nu vergelijken met de performantie de we verkregen hadden tijdens het zoeken in de suffixboom zien we dat dit terug in elkaars buurt komt.
Terwijl we in de suffixboom de LCA al voorberekend hadden.

\section{UniProtKB}
Zoals eerder vermeld levert een ruwe extrapolatie op dat we geschat 1.2 TB aan geheugen nodig zou hebben.
Hierbij werd echter uitgegaan van het feit dat UniProtKB 500x meer eiwitten dan Swiss-Prot bevat.
Dit is een overschatting van de realiteit waar UniProt \textit{slechts} $\pm$ 440 keer groter is. % TODO: check deze factor.
...
Figuur~\ref{fig:build_uniprot} toont de nodige hoeveelheid geheugen en tijd.
...
Nu we wisten dat het haalbaar was om een suffix array op te bouwen voor de volledige uniProt databank was een volgende stap het kiezen van de sparseness factor.
...
Dit resulteert in een SA van TODO GB.
In combinatie met de tekst resulteert dit in een totale indexgrootte van TODO GB, wat comfortabel in de 0.5 TB past.
Indien we sparseness factor 2 zouden gebruiken is dit niet meer het geval. Dan komt de totale indexgrootte op $\pm$ 0.5 TB uit, wat te dicht ligt bij de grens.
