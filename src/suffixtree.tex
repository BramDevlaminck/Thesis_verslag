...
Meer specifiek gaat een variant hiervan, de zogenaamde veralgemeende suffixboom.
Deze laten het toe om efficiënt te controleren als een string deel is van een \textbf{verzameling} van andere strings.
\\ \\
We behandelen deze datastructuur als eerste omdat hij vrij intuïtief is en het minst complex is.
...
Hiervoor hebben we een \textbf{eigen Rust implementatie}.
...
Suffixbomen zijn een soort tries.
Tries zijn boomstructuren waarbij elk pad tot een blad een string voorstelt.
Meer specifiek zijn het Patricia\footnote{Dit is een trie waarbij elke interne top een vertakking voorstelt. Anders gezegd: er zijn geen interne toppen die maar 1 kind hebben.} tries\cite{patricia} waarbij het laatste teken van de inputstring uniek is binnen die string.
Dit unieke eindteken zal ervoor zorgen dat elke suffix van de inputstring uniek is.
Elke suffix is dus nooit de prefix van een andere suffix.
Bijgevolg zal \textbf{elke suffix een eigen blad in de boom} krijgen.
...
\begin{figure}[H]
    ...
    [
        ...
        ;
        \draw[dashed, ->] (Start) to[out=-20,in=200] (End);
    \end{tikzpicture}
...
Op deze manier wordt de nodige opslag per top een constante en krijgen we \textbf{lineair geheugenverbruik}.
...
Deze suffixbomen laten toe om in één string snel substrings te zoeken.
Zoals eerder vermeld willen we echter peptides zoeken in een \textbf{verzameling van proteïnes}.
Daarom wordt gebruik gemaakt van \textbf{veralgemeende suffixbomen}.
Hierbij wordt de verzameling van woorden die geïndexeerd moet worden behandeld alsof het één grote tekst is.
De manier waarop deze tekst gevormd wordt is echter erg belangrijk.
Alle proteïnes moeten aan elkaar gehangen worden met ertussen een teken dat niet in de proteïnes en peptides zelf voorkomt, en als eindteken moet er een ander uniek tegen gebruikt worden.
Wanneer niet aan de eerste voorwaarde voldaan wordt zullen we ongeldige matches vinden.
Het zou namelijk mogelijk zijn om een match te vinden die over 2 proteïnes gespreid is.
...
Hierbij stapt Rust af van zijn standaard \textit{ownership} systeem wordt gebruik gemaakt van \textbf{Reference Counting}~\cite{reference_counting}.
...
Namelijk bij \textbf{cyclische verwijzingen}; een patroon dat jammer genoeg erg vaak voor komt.
...
\\ \\
Aangezien we alles in één vector opslaan zou het mogelijk zijn de nodige hoeveelheid geheugen onmiddellijk aan te vragen bij het opstarten van het programma.
...
In de praktijk worden er echter slechts 328 922 516 toppen gevormd.
Indien we het geheugen op voorhand aanvragen zou het geheugenverbruik dus nog met een factor $\frac{413 047 385}{328 922 516} \approx 1,26$ hoger liggen.
...
Kinderen worden dus voorgesteld aan de hand van een array die een vaste grootte heeft, wat als gevolg heeft dat \textbf{elke top even groot is}.
...
Bijna exact \textbf{2x zo hoog als de equivalente C++ implementatie}.
...
Als oplossing heb ik uiteindelijk mijn \textbf{eigen \textit{null} waarde gedefinieerd} die gebruik maakt van een \textit{trait}\footnote{Een trait in Rust definieert een functionaliteit dat een bepaald type heeft, en kan delen met andere types}.
...
Bovendien blijft \textbf{memory safety gegarandeerd}, aangezien het foutief indexeren van de null value (\texttt{usize::MAX} in dit geval) een index-out-of-bounds error creëert.
...
Uiteindelijk ben ik gegaan voor een implementatie (zowel in Rust als C++) waarin plaatsgehouden wordt voor \textbf{28 kinderen}.
...
Hoe dan ook blijft het \textbf{geheugenverbruik extreem groot}, welke implementatie ook gekozen wordt.
\\ \\
...
De uitvoeringstijd en het geheugenverbruik zijn gemeten aan de hand van het unix \texttt{time} commando.
...
Unipept kiest echter een andere aanpak waarbij de informatie \textbf{conservatief} veralgemeend wordt omdat er geen manier is om met zekerheid te zeggen uit welke proteïne de peptide effectief komt.
...
Voor de Swiss-Prot databank gaat het geheugenverbruik al boven 85 GB als we ook de taxon IDs voorberekenen, terwijl ons einddoel is om de indexstructuur op te bouwen voor de volledige UniProtKB databank.
