...
\begin{enumerate}
    ...
    \item Voor alle bogen die uit een bepaalde top vertrekken geldt dat het eerste karakter van hun labels verschillend is.
...
\\ \\
Het navigeren in een suffixboom kunnen we efficiënter maken door ook de \textbf{suffixlinks} bij te houden.
Dit is een pointer van een interne top $v$ naar een andere interne top $s(v)$.
Deze suffixlink bestaat als het pad uit de wortel naar top $v$ voorgesteld kan worden door de label $c\alpha$, waar $c$ een karakter is en $\alpha$ een (mogelijks lege) string.
...
Hierbij stellen de stippellijnen de suffixlinks voor.
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Suffixboom voor de string \texttt{acacgt\$}. De stippellijnen stellen de suffixlinks voor.}\label{fig:suffix_tree_example}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Suffixboom voor de string \texttt{acacgt\$}, gebruik makende van indices. De stippellijnen stellen de suffixlinks voor.}\label{fig:suffix_tree_example_indices}

\end{figure}

Deze suffixbomen laten toe om in één string in $O(n)$ tijd substrings te zoeken.
