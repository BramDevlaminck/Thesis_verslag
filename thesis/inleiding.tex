...
\begin{itemize}
    ...
    \item \textbf{Unipept CLI\footnote{\textit{command-line interface}}~\cite{unipept_cli}} Dit is een \textit{power-user} tool om analyses uit te voeren via de commandolijn op grotere stalen en laat toe om de Unipept analyses in een grotere \textit{pipeline} op te nemen.
...
In deze masterproef zoeken we een oplossing voor het snel terugvinden van \textbf{willekeurige peptiden}\footnote{Hiermee bedoelen we peptiden die tryptisch zouden moeten zijn, maar één of meerdere gemiste splitsingen hebben. Daarnaast omvat dit ook peptiden die niet-tryptisch zijn, en dus op een andere manier gesplitst worden.} in een eiwitdatabank.
...
Deze zij ook allemaal afkomstig uit UniProtKB 2023\_04.
\\ \\
...
\\ \\
We hebben een grote databank met erg veel proteïnes (een lange tekst) waarin we erg veel peptiden (korte strings) zoeken, wat overeenkomt met de tweede aanpak.
