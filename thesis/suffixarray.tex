...
Traditionele sorteeralgoritmen zoals merge sort~\cite{mergeSort} kunnen in $O(n^2 \log n)$ tijd en $O(n^2)$ geheugen een suffix array opbouwen.
Hierbij is $n$ de lengte van de tekst.
De totale tijdscomplexiteit is $O(n^2 \log n)$ en niet $O(n \log n)$ aangezien elke vergelijking van twee suffixen in het slechtste geval $n$ karaktervergelijkingen vereist.
