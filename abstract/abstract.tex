...
%        \raggedcolumns
        ...
        Because of this, performing the analyses at runtime can become very slow.
        Furthermore, in most cases the analyses for such peptides yields very a generic result.
        ...
        The resulting suffix array is around 700 GB in size (+ another 88 GB for the text).
        ...
        Unipept 6.x supports two types of search with the new index structure.
        Most importantly, using the new index we can efficiently search and perform the taxonomic and functional analyses at runtime.
        Additionally, it is now possible to retrieve all information about the matching proteins without any analysis.
        Both options perform similar, but surprisingly the first option is slightly faster if the second option also retrieves the functional annotations.
        This is caused by the extra data (around ten times as much) that needs to be serialized to a JSON at the end of the request.
        In this section we only discuss the search with analyses, since this is what Unipept 5.x supports and the other search performs very similar.
        ...
        From our benchmarks, Unipept 6.x strongly outperforms its closest competitors at finding all occurrences for an arbitrary peptide longer than 2 amino acids in UniProtKB\@.

        Furthermore, the new index structure equates I and L during search without a performance impact.
        Whenever we do not want to equate these amino acids, this is also possible with a small performance hit.
